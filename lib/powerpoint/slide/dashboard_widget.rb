...
module Powerpoint
  module Slide
    class DashboardWidget
      ...
      def default_coords
        start_x = pixle_to_pt(30)
        start_y = pixle_to_pt(20)
        ...
        {x: start_x, y: pixle_to_pt(140), cx: new_width, cy: new_height}
      end
      private :default_coords

      def save_rel_xml(extract_path, index)
        render_view('dashboard_widget_rel.xml.erb', "#{extract_path}/ppt/slides/_rels/slide#{index}.xml.rels")
      end
      private :save_rel_xml

      def save_slide_xml(extract_path, index)
        render_view('dashboard_widget_slide.xml.erb', "#{extract_path}/ppt/slides/slide#{index}.xml")
      end
